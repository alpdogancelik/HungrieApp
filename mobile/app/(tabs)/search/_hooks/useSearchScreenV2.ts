// /(tabs)/search/_hooks/useSearchScreenV3.ts
...
const getMealKeyFromItem = (item: any) => {
  ...
  // en kötü senaryo: name + price ile stabil anahtar
...
export const useSearchScreenV3 = () => {
  ...
  // Meals: relevance sırası bozulmasın
  const mealsFlat = useMemo(() => results, [results]);

  // Restaurants: query varsa local filtre (useSearch filtrelemese bile garanti)
        ...
        for (let i = 0; i < diff; i += 1) {
          addItem({
            ...
            image_url: safeImageUrl(
              (item as any).image_url || (item as any).imageUrl || (item as any).image || (item as any).photo || "",
            ),
