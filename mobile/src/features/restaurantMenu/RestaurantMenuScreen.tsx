...
const RestaurantMenuScreen = () => {
    ...
    }), [selected, selectedCustomizationIds]);
    const toggleCustomization = useCallback((option: any) => setSelectedCustomizationIds((current) => {
        if (current.includes(option.id)) return current.filter((id) => id !== option.id);
        const withoutGroup = option.groupKind === "single" && option.groupId
            ? current.filter((id) => !(selected?.item.customizations || []).some((candidate) => candidate.id === id && candidate.groupId === option.groupId))
            : current;
        const groupCount = (selected?.item.customizations || []).filter((candidate) => candidate.groupId === option.groupId && withoutGroup.includes(candidate.id)).length;
        if (option.groupId && groupCount >= Number(option.maximumSelections || Number.MAX_SAFE_INTEGER)) return withoutGroup;
