...
export default function OrderDetailsScreen({ orderId }: Props) {
    ...
    return <SafeAreaView edges={["top", "left", "right"]} style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={[styles.content, { paddingBottom: Math.max(insets.bottom, 18) + 24 }]} showsVerticalScrollIndicator={false}>
            ...
            {delivered ? <Pressable accessibilityLabel={copy.reorder} accessibilityRole="button" disabled={reordering} onPress={() => void reorder()} style={styles.reorder}>{reordering ? <ActivityIndicator color={ORANGE} size="small" /> : <Ionicons color={ORANGE} name="refresh-outline" size={19} />}<Text style={styles.reorderText}>{reordering ? copy.adding : copy.reorder}</Text></Pressable> : null}
        </ScrollView>
