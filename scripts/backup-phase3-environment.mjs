...
if (!['development', 'staging'].includes(target) || arg('--confirm') !== `${target}:phase3-backup`) {
  throw new Error('Use --target=development|staging with matching --confirm=<target>:phase3-backup.');
}
...
const destination = path.join(secure, `phase3-${target}-backup`, new Date().toISOString().replaceAll(/[:.]/g, '-'));
