...
const migrationName = "20260917100000_customer_review_system_v2.sql";
const migrationPath = path.join(root, "supabase", "migrations", migrationName);
const expectedSha = "5340eeb74a50c038ae755b812ed5efcef5a29bd84b2251c4d66bff7acf5f372b";
const actualSha = crypto.createHash("sha256").update(fs.readFileSync(migrationPath)).digest("hex");
const phaseDir = path.join(secure, "customer-review-v2-phase2");
...
if (!project?.ref || !project.databasePassword || project.name !== "HungrieApp Development" ||
    ...
    !project.url || !project.publishableKey || !managementToken) {
  throw new Error("Safe isolated Development configuration is required.");
}
if (actualSha !== expectedSha || arg("--expect-sha256") !== expectedSha) {
  throw new Error(`Reviewed migration SHA-256 is required: ${expectedSha}`);
}
if (!["backup", "preflight", "apply", "probe", "plans", "types", "verify-cleanup"].includes(action)) {
  throw new Error("Use backup|preflight|apply|probe|plans|types|verify-cleanup with the reviewed SHA-256.");
}

fs.mkdirSync(phaseDir, { recursive: true, mode: 0o700 });
fs.chmodSync(phaseDir, 0o700);
...
const query = async (sql) => {
  ...
  if (!response.ok) throw new Error(`Development SQL request failed (${response.status}); details withheld.`);
  const rows = await response.json();
  if (!Array.isArray(rows)) throw new Error("Development SQL response was malformed.");
  return rows;
};
const metadata = async () => {
  const response = await fetch(`https://api.supabase.com/v1/projects/${project.ref}`, {
    headers: { authorization: `Bearer ${managementToken}` },
  });
  if (!response.ok) throw new Error(`Development metadata request failed (${response.status}).`);
  const value = await response.json();
  if (value.name !== "HungrieApp Development" || value.region !== "eu-central-1" ||
      value.status !== "ACTIVE_HEALTHY" || !String(value.database?.version || "").startsWith("17")) {
    throw new Error("Development identity, region, health, or PostgreSQL version changed.");
  }
  return value;
};
const snapshotSql = `select
  ...
  (select count(*)::integer from public.order_reviews where status='published') order_published,
  (select count(*)::integer from public.order_reviews where status='hidden') order_hidden,
  ...
  (select encode(extensions.digest(coalesce(jsonb_agg(jsonb_build_array(id,review_key,order_id,restaurant_id,profile_id,
    user_name_snapshot,restaurant_name_snapshot,speed_rating,taste_rating,value_rating,price_performance_rating,
    average_rating,comment,items_snapshot,status,created_at,updated_at) order by id)::text,'[]'),'sha256'),'hex') from public.order_reviews) order_digest,
...
const history = async () => query("select version from supabase_migrations.schema_migrations order by version");
const dryRun = () => {
  const result = run("supabase", ["db", "push", "--project-ref", project.ref, "--password", project.databasePassword,
    "--skip-vault", "--dry-run"], { env: cliEnv });
  const output = `${result.stdout || ""}\n${result.stderr || ""}`;
  if (result.status !== 0 || !output.includes(migrationName)) throw new Error("Development migration dry-run failed.");
  const listed = [...output.matchAll(/\d{14}_[A-Za-z0-9_]+\.sql/g)].map((match) => match[0]);
  if ([...new Set(listed)].some((name) => name !== migrationName)) throw new Error("Dry-run included an unreviewed migration.");
  return true;
};
const preflight = async ({ persist = true } = {}) => {
  await metadata();
  const versions = await history();
  if (versions.length !== 45 || String(versions.at(-1)?.version) !== "20260916130000") {
    throw new Error("Development migration history no longer matches the 45-migration Phase 0 baseline.");
  }
  ...
  if (pending.length !== 1 || pending[0] !== migrationName) throw new Error("Pending migration set changed.");
  const before = await snapshot();
  if ([before.product_reviews,before.order_reviews,before.product_duplicates,before.order_duplicates].some(Number)) {
    throw new Error("Development review baseline changed since Phase 0; refusing to migrate.");
  }
  const functions = await query(`select proname,pg_get_functiondef(p.oid) definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and proname in ('list_my_product_reviews','list_my_order_reviews') order by proname`);
  if (functions.length !== 2 || functions.some((entry) => !/select \*/i.test(entry.definition))) {
    throw new Error("Expected Development-only broad legacy helper drift was not present.");
  }
  dryRun();
  const record = { environment: "development", projectRef: project.ref, capturedAt: new Date().toISOString(),
    migration: migrationName, migrationSha256: expectedSha, migrationCount: 45, latestMigration: "20260916130000",
    snapshot: before, driftDefinitions: Object.fromEntries(functions.map((entry) => [entry.proname, sha256(entry.definition)])) };
  if (persist) fs.writeFileSync(baselinePath, JSON.stringify(record, null, 2), { mode: 0o600 });
  return record;
};

if (action === "backup") {
  if (arg("--confirm") !== "development:review-v2-backup") throw new Error("Backup confirmation mismatch.");
  await metadata();
  const directory = path.join(phaseDir, `backup-${new Date().toISOString().replaceAll(/[:.]/g, "-")}`);
  ...
  for (const entry of entries) {
    ...
    const result = run("supabase", ["db", "dump", "--project-ref", project.ref, "--password", project.databasePassword,
      "--file", output, ...entry.args], { env: cliEnv });
    if (result.status !== 0 || !fs.existsSync(output) || fs.statSync(output).size < 1024) throw new Error("Restricted Development backup failed.");
    fs.chmodSync(output, 0o600);
  }
  const files = entries.map(({ name }) => { const file = path.join(directory,name); return { name, bytes: fs.statSync(file).size, sha256: sha256(fs.readFileSync(file)) }; });
  const manifest = { environment: "development", projectRef: project.ref, recordedAt: new Date().toISOString(), files };
  const manifestPath = path.join(directory, "manifest.json");
  fs.writeFileSync(manifestPath, JSON.stringify(manifest,null,2), { mode: 0o600, flag: "wx" });
  console.log(JSON.stringify({ action, environment: "development", manifestPath, files }));
} else if (action === "preflight") {
  if (arg("--confirm") !== "development:review-v2-preflight") throw new Error("Preflight confirmation mismatch.");
  const result = await preflight();
  console.log(JSON.stringify({ action, environment: "development", migration: result.migration, migrationSha256: expectedSha,
    migrationCount: result.migrationCount, baseline: result.snapshot, dryRun: true }));
} else if (action === "apply") {
  if (arg("--confirm") !== "development:review-v2-migration") throw new Error("Migration confirmation mismatch.");
  const before = await preflight();
  const manifestPath = path.resolve(arg("--backup-manifest") || "");
  if (!manifestPath.startsWith(phaseDir + path.sep) || !fs.existsSync(manifestPath)) throw new Error("Phase 2 backup manifest is required.");
  const manifest = JSON.parse(fs.readFileSync(manifestPath,"utf8"));
  if (manifest.environment !== "development" || manifest.projectRef !== project.ref ||
      Date.now()-Date.parse(manifest.recordedAt)>86_400_000) throw new Error("Backup manifest is stale or mismatched.");
  for (const entry of manifest.files || []) {
    const file = path.join(path.dirname(manifestPath),entry.name);
    if (!fs.existsSync(file) || sha256(fs.readFileSync(file)) !== entry.sha256) throw new Error("Backup checksum verification failed.");
  }
  const pushed = run("supabase", ["db","push","--project-ref",project.ref,"--password",project.databasePassword,"--skip-vault","--yes"], { env: cliEnv });
  if (pushed.status !== 0) throw new Error("Development migration application failed; operator output withheld.");
  const versions = await history();
  const after = await snapshot();
  if (versions.length!==46 || String(versions.at(-1)?.version)!=="20260917100000" ||
      before.snapshot.order_digest!==after.order_digest || before.snapshot.product_digest!==after.product_digest ||
      Number(after.order_reviews)!==0 || Number(after.product_reviews)!==0) throw new Error("Post-migration history or review reconciliation failed.");
  const [catalog] = await query(`select
    (select count(*)=4 from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='private' and c.relname in
      ('order_review_meal_reactions','customer_review_operations','order_review_reports','review_action_operations') and c.relrowsecurity and c.relforcerowsecurity) forced_rls,
    (select count(*)=2 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in
      ('get_restaurant_review_summary_v2','list_published_restaurant_reviews_v2')) public_reads,
    (select count(*)=5 from pg_indexes where indexname in ('order_reviews_restaurant_status_created_id_idx','order_reviews_published_feed_idx',
      'order_review_reports_restaurant_status_created_id_idx','order_review_reports_status_created_id_idx','order_review_reactions_restaurant_item_reaction_idx')) indexes_ok`);
  if (!catalog?.forced_rls || !catalog?.public_reads || !catalog?.indexes_ok) throw new Error("Post-migration catalog verification failed.");
  fs.writeFileSync(path.join(phaseDir,"development-applied.json"),JSON.stringify({ appliedAt:new Date().toISOString(),migration:migrationName,
    migrationSha256:expectedSha,before:before.snapshot,after,backupManifest:manifestPath },null,2),{mode:0o600});
  console.log(JSON.stringify({ action,environment:"development",migration:migrationName,migrationSha256:expectedSha,
    migrationCount:46,reconciled:true,forcedRls:true,indexes:true }));
} else if (action === "types") {
  if (arg("--confirm") !== "development:review-v2-types") throw new Error("Type-generation confirmation mismatch.");
  const versions = await history();
  if (versions.length!==46 || String(versions.at(-1)?.version)!=="20260917100000") throw new Error("Development migration is not qualified for type generation.");
  const hosted = run("supabase",["gen","types","typescript","--project-id",project.ref,"--schema","public"],{env:cliEnv});
  const local = run("supabase",["gen","types","typescript","--local","--schema","public"]);
  const publicSchema = (value) => value.slice(value.indexOf("  public: {"),value.indexOf("\ntype DatabaseWithoutInternals"));
  if (hosted.status!==0 || local.status!==0 || !hosted.stdout ||
      !publicSchema(hosted.stdout) || publicSchema(hosted.stdout)!==publicSchema(local.stdout)) {
    throw new Error("Hosted and local generated public schema types do not match.");
  }
  const output = path.join(root,"packages/database-types/src/database.generated.ts");
  fs.writeFileSync(output,hosted.stdout,"utf8");
  console.log(JSON.stringify({action,environment:"development",bytes:Buffer.byteLength(hosted.stdout),sha256:sha256(hosted.stdout),
    publicSchemaSha256:sha256(publicSchema(hosted.stdout)),localSchemaParity:true,generatorEnvelopeParity:hosted.stdout===local.stdout}));
} else if (action === "verify-cleanup") {
  if (arg("--confirm") !== "development:review-v2-cleanup") throw new Error("Cleanup confirmation mismatch.");
  const baseline = JSON.parse(fs.readFileSync(baselinePath,"utf8"));
  const current = await snapshot();
  const [probe] = await query(`select
    (select count(*) from public.profiles where id like 'crv2p2_%')::integer profiles,
    (select count(*) from public.orders where id like 'crv2p2_%')::integer orders,
    (select count(*) from public.order_reviews where id like 'crv2p2_%' or order_id like 'crv2p2_%')::integer reviews,
    (select count(*) from private.order_review_reports where review_id like 'crv2p2_%')::integer reports,
    (select count(*) from private.order_review_meal_reactions where review_id like 'crv2p2_%')::integer reactions,
    (select count(*) from private.audit_log where target_id like 'crv2p2_%' or metadata::text like '%crv2p2_%')::integer audits`);
  const legacyFunctions = await query(`select proname,pg_get_functiondef(p.oid) definition from pg_proc p
    join pg_namespace n on n.oid=p.pronamespace where n.nspname='public'
    and proname in ('list_my_product_reviews','list_my_order_reviews') order by proname`);
  const [security] = await query(`select
    (select count(*)=4 from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='private'
      and c.relname in ('order_review_meal_reactions','customer_review_operations','order_review_reports','review_action_operations')
      and c.relrowsecurity and c.relforcerowsecurity) forced_rls,
    (select count(*)=2 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public'
      and p.proname in ('get_restaurant_review_summary_v2','list_published_restaurant_reviews_v2')
      and has_function_privilege('anon',p.oid,'execute') and has_function_privilege('authenticated',p.oid,'execute')) public_grants,
    (select count(*)=10 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public'
      and p.proname in ('get_my_customer_order_review_state_v2','get_my_customer_review_prompt_v2','restaurant_list_order_reviews_v2',
        'restaurant_report_order_review_v2','restaurant_list_menu_item_reaction_aggregates_v2','submit_my_customer_order_review_v2',
        'admin_list_order_review_reports_v2','admin_set_order_review_report_status_v2','admin_set_order_review_visibility_v2',
        'admin_list_menu_item_reaction_aggregates_v2')
      and not has_function_privilege('anon',p.oid,'execute') and has_function_privilege('authenticated',p.oid,'execute')) authenticated_only_grants,
    not exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='private'
      and p.proname like 'review_v2_%' and (has_function_privilege('anon',p.oid,'execute') or has_function_privilege('authenticated',p.oid,'execute'))) private_helpers_closed,
    not exists(select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='private'
      and c.relname in ('order_review_meal_reactions','customer_review_operations','order_review_reports','review_action_operations')
      and (has_table_privilege('anon',c.oid,'select,insert,update,delete') or has_table_privilege('authenticated',c.oid,'select,insert,update,delete'))) private_relations_closed`);
  if (Object.values(probe).some(Number) || baseline.snapshot.order_digest!==current.order_digest ||
      baseline.snapshot.product_digest!==current.product_digest || Number(current.order_reviews)!==0 || Number(current.product_reviews)!==0 ||
      legacyFunctions.length!==2 || legacyFunctions.some(({definition})=>/select\s+\*/i.test(definition)) ||
      Object.values(security).some(value=>value!==true)) {
    throw new Error("Development cleanup or baseline reconciliation failed.");
  }
  console.log(JSON.stringify({action,environment:"development",probeRows:probe,reviewBaselineReconciled:true,
    legacyProjectionHashes:Object.fromEntries(legacyFunctions.map(({proname,definition})=>[proname,sha256(definition)])),security}));
}

// probe and plans are appended below to keep all hosted qualification behind this guarded entrypoint.

if (action === "probe") {
  if (arg("--confirm") !== "development:review-v2-probe") throw new Error("Probe confirmation mismatch.");
  ...
  const versions=await history();
  if(versions.length!==46||String(versions.at(-1)?.version)!=="20260917100000")throw new Error("Development v2 migration is unavailable.");
  ...
  const app=admin.initializeApp({credential:admin.credential.cert(credential),projectId:credential.project_id},`crv2p2-${Date.now()}`),auth=app.auth();
  const suffix=crypto.randomUUID().replaceAll("-","").slice(0,12),prefix=`crv2p2_${suffix}`;
  const ids={customerA:`${prefix}_ca`,customerB:`${prefix}_cb`,restaurantA:`${prefix}_ra`,restaurantB:`${prefix}_rb`,suspended:`${prefix}_sus`,revoked:`${prefix}_rev`,admin:`${prefix}_admin`,category:`${prefix}_cat`,item:`${prefix}_item`,main:`${prefix}_main`,other:`${prefix}_other`,legacy:`${prefix}_legacy`,expired:`${prefix}_expired`};
  const users={},tokens={};let fixtures=false,restaurantA,restaurantB,mainReview,reportId;
  const op={submit:crypto.randomUUID(),submitOther:crypto.randomUUID(),report:crypto.randomUUID(),reportDuplicate:crypto.randomUUID(),resolve:crypto.randomUUID(),reopen1:crypto.randomUUID(),dismiss:crypto.randomUUID(),reopen2:crypto.randomUUID(),hide:crypto.randomUUID(),restore:crypto.randomUUID()};
  try{
    ...
    await query(`begin;
      ...
      insert into public.menu_items(id,restaurant_id,category_id,name,price_kurus,is_active,sort_order)
        values(${quote(ids.item)},${quote(restaurantA)},${quote(ids.category)},'Server-owned probe meal',100,true,0);
      ...
      insert into public.order_items(id,order_id,menu_item_id,source_menu_item_id,name_snapshot,unit_price_kurus,quantity,customizations_snapshot,created_at)values
        (${quote(`${ids.main}_1`)},${quote(ids.main)},${quote(ids.item)},${quote(ids.item)},'Server-owned probe meal',100,1,'[]',transaction_timestamp()-interval '2 seconds'),
        (${quote(`${ids.main}_2`)},${quote(ids.main)},${quote(ids.item)},${quote(ids.item)},'Server-owned probe meal',100,2,'[{"probe":"configured"}]',transaction_timestamp()-interval '1 second'),
    ...
    const submitted=await must("rpc/submit_my_customer_order_review_v2",tokens.customerA,{p_order_id:ids.main,p_taste_rating:4,p_speed_rating:5,p_comment:"  café\tline\n  ",p_meal_reactions_json:[{menuItemId:ids.item,reaction:"liked"}],p_operation_id:op.submit});mainReview=submitted.reviewId;if(!mainReview||submitted.replayed!==false)throw new Error("Initial v2 submission result was invalid.");
    const replay=await must("rpc/submit_my_customer_order_review_v2",tokens.customerA,{p_order_id:ids.main,p_taste_rating:4,p_speed_rating:5,p_comment:"café\tline\n",p_meal_reactions_json:[{reaction:"liked",menuItemId:ids.item}],p_operation_id:op.submit});if(replay.reviewId!==mainReview||replay.replayed!==true)throw new Error("Exact canonical replay failed.");
    ...
    if(!rows?.v2?.v2Nulls||rows.v2.groupedQuantity!=="3"||rows.v2.serverName!=="Server-owned probe meal"||rows.v2.comment!=="café\tline\n")throw new Error("Stored v2 semantics failed.");
    ...
    const summary=await must("rpc/get_restaurant_review_summary_v2",null,{p_restaurant_id:restaurantA});if(Number(summary.reviewCount)!==3||Number(summary.overallRating)!==4)throw new Error("Database-owned v2 summary was incorrect.");
    ...
    const aggregate=await must("rpc/restaurant_list_menu_item_reaction_aggregates_v2",tokens.restaurantA,{p_cursor:null,p_limit:50});const itemAgg=aggregate.items.find(x=>x.menuItemId===ids.item);if(!itemAgg||Number(itemAgg.likedCount)!==1||Number(itemAgg.dislikedCount)!==1||Number(itemAgg.positivePercentage)!==50||forbiddenKeys.some(k=>JSON.stringify(aggregate).includes(`\"${k}\"`)))throw new Error("Restaurant reaction aggregate failed privacy or count checks.");
    ...
    const adminInspection=await must("rpc/admin_list_order_review_reports_v2",tokens.admin,{p_status:null,p_restaurant_id:restaurantA,p_cursor:null,p_limit:20});if(forbiddenKeys.slice(0,6).some(k=>JSON.stringify(adminInspection).includes(`\"${k}\"`)))throw new Error("Admin inspection exposed Customer identity or order PII.");
    ...
    console.log(JSON.stringify({action,environment:"development",activeCustomer:true,canonicalReplay:true,changedReplayDenied:true,ownershipIsolation:true,statusDenials:true,utcBoundary:true,v1Compatibility:true,publicPrivacy:true,keysetTieBreak:true,restaurantReportOnly:true,reportReplay:true,aggregatePrivacy:true,realMfaAdmin:true,firstFactorDenied:true,staleAuthDenied:true,reportTransitions:true,hideRestoreTransactional:true,auditSanitized:true}));
  }finally{
    if(fixtures){await query(`begin;
      ...
      delete from public.menu_items where id=${quote(ids.item)};delete from public.categories where id=${quote(ids.category)};
    ...
    for(const user of Object.values(users))if(user?.uid)await auth.deleteUser(user.uid).catch(()=>{});
    await app.delete();
  }
}

if(action==="plans"){
  if(arg("--confirm")!=="development:review-v2-plans")throw new Error("Query-plan confirmation mismatch.");
  const versions=await history();if(versions.length!==46||String(versions.at(-1)?.version)!=="20260917100000")throw new Error("Development v2 migration is unavailable.");
  const planSql=`begin;
    create temporary table crv2p2_restaurants as select row_number()over(order by id)::integer n,id from public.restaurants where lifecycle_status='active' order by id limit 9;
    do $$begin if (select count(*) from crv2p2_restaurants)<2 then raise exception 'representative plans require two Restaurants';end if;end$$;
    create temporary table crv2p2_settings as select count(*)::integer restaurant_count,(select id from crv2p2_restaurants where n=1) target_restaurant from crv2p2_restaurants;
    insert into public.profiles(id,name,email,preferred_language)
      select 'crv2p2_plan_profile_'||g,'Plan fixture','crv2p2-plan-'||g||'@example.invalid','en' from generate_series(1,5000)g;
    insert into public.orders(id,profile_id,restaurant_id,status,payment_method,subtotal_kurus,total_kurus,delivered_at,created_at)
      select 'crv2p2_plan_order_'||g,'crv2p2_plan_profile_'||g,r.id,'delivered','cash',100,100,
        transaction_timestamp()-interval '1 hour',transaction_timestamp()-(g||' seconds')::interval
      from generate_series(1,5000)g join crv2p2_settings s on true join crv2p2_restaurants r on r.n=1+((g-1)%s.restaurant_count);
    insert into public.order_reviews(id,review_key,order_id,restaurant_id,profile_id,user_name_snapshot,restaurant_name_snapshot,
      ...
      select 'crv2p2_plan_review_'||g,'crv2p2_plan_key_'||g,'crv2p2_plan_order_'||g,r.id,'crv2p2_plan_profile_'||g,
    ...
    insert into private.order_review_meal_reactions(review_id,order_id,restaurant_id,profile_id,menu_item_id,reaction,menu_item_name_snapshot)
      select 'crv2p2_plan_review_'||g,'crv2p2_plan_order_'||g,r.id,'crv2p2_plan_profile_'||g,
        'crv2p2_plan_item_'||(g%100),case when g%2=0 then 'liked'::private.meal_reaction else 'disliked'::private.meal_reaction end,'Plan Item'
      from generate_series(1,5000)g join crv2p2_settings s on true join crv2p2_restaurants r on r.n=1+((g-1)%s.restaurant_count);
    insert into private.customer_review_operations(profile_id,operation_id,request_sha256,review_id)
      select 'crv2p2_plan_profile_'||g,md5('crv2p2-operation-'||g)::uuid,repeat('a',64),'crv2p2_plan_review_'||g from generate_series(1,5000)g;
    insert into private.order_review_reports(review_id,restaurant_id,created_by_profile_id,reason,status,created_at)
      select 'crv2p2_plan_review_'||g,r.id,'crv2p2_plan_profile_'||g,'spam',
      ...
      from generate_series(1,1500)g join crv2p2_settings s on true join crv2p2_restaurants r on r.n=1+((g-1)%s.restaurant_count);
    analyze public.profiles;analyze public.orders;analyze public.order_reviews;analyze private.order_review_meal_reactions;
    analyze private.customer_review_operations;analyze private.order_review_reports;
    create temporary table crv2p2_plans(name text primary key,expected_index text not null,plan jsonb not null);
    do $$declare p jsonb;target text:=(select target_restaurant from crv2p2_settings);begin
      ...
      insert into crv2p2_plans values('public_feed','order_reviews_published_feed_idx',p);
      execute 'explain(analyze,buffers,format json)select id from public.order_reviews where order_id=''crv2p2_plan_order_4999'' and profile_id=''crv2p2_plan_profile_4999'''into p;
      insert into crv2p2_plans values('customer_lookup','order_reviews_profile_created_idx',p);
      ...
      insert into crv2p2_plans values('restaurant_queue','order_reviews_restaurant_status_created_id_idx',p);
      ...
      insert into crv2p2_plans values('report_queue','order_review_reports_restaurant_status_created_id_idx',p);
      execute 'explain(analyze,buffers,format json)select review_id from private.customer_review_operations where profile_id=''crv2p2_plan_profile_4999'' and operation_id=md5(''crv2p2-operation-4999'')::uuid'into p;
      insert into crv2p2_plans values('operation_replay','customer_review_operations_pkey',p);
      ...
      insert into crv2p2_plans values('reaction_aggregate','order_review_reactions_restaurant_item_reaction_idx',p);
    end$$;
    select name,expected_index,plan::text like '%'||expected_index||'%' uses_expected_index,
      case when name in('public_feed','customer_lookup','restaurant_queue')then plan::text like '%\"Node Type\": \"Seq Scan\"%\"Relation Name\": \"order_reviews\"%'
        when name='report_queue'then plan::text like '%\"Node Type\": \"Seq Scan\"%\"Relation Name\": \"order_review_reports\"%'
        when name='operation_replay'then plan::text like '%\"Node Type\": \"Seq Scan\"%\"Relation Name\": \"customer_review_operations\"%'
        else plan::text like '%\"Node Type\": \"Seq Scan\"%\"Relation Name\": \"order_review_meal_reactions\"%' end large_seq_scan,
      plan from crv2p2_plans order by name;
    rollback;`;
  const plans=await query(planSql);
  const output=path.join(phaseDir,"development-query-plans.json");fs.writeFileSync(output,JSON.stringify({environment:"development",capturedAt:new Date().toISOString(),volume:{reviews:5000,reactions:5000,reports:1500,operations:5000},plans},null,2),{mode:0o600});
  if(plans.length!==6||plans.some(row=>!row.uses_expected_index||row.large_seq_scan))throw new Error(`Representative query-plan gate failed: ${plans.map(row=>`${row.name}:${row.uses_expected_index}:${row.large_seq_scan}`).join(",")}; sanitized plans saved for review.`);
  console.log(JSON.stringify({action,environment:"development",volume:{reviews:5000,reactions:5000,reports:1500,operations:5000},plans:plans.map(({name,expected_index,uses_expected_index,large_seq_scan})=>({name,expectedIndex:expected_index,usesExpectedIndex:uses_expected_index,largeSeqScan:large_seq_scan})),rolledBack:true,evidencePath:output}));
}
