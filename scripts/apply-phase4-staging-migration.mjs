...
if (arg("--target") !== "staging" || (apply && arg("--confirm") !== "staging:phase4-migration")) throw new Error("Target staging explicitly; apply requires --confirm=staging:phase4-migration.");
...
if (history.length !== 30 || pending.length !== 1 || pending[0] !== migration) throw new Error("Expected 30 applied migrations and only Phase 4 pending.");
if (apply) {
...
const [after] = await query("select (select count(*) from supabase_migrations.schema_migrations)::int migrations,(select count(*) from private.account_access)::int accounts,(select count(*) from private.user_roles)::int legacy_admin_roles");
if (after?.migrations !== 31 || after.accounts !== 9 || after.legacy_admin_roles !== 0) throw new Error("Staging post-migration invariant failed.");
