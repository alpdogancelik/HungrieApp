...
const secure=path.join(root,"secure"),phaseDir=path.join(secure,"customer-review-v2-phase8");
const registry=JSON.parse(fs.readFileSync(path.join(secure,"supabase-projects.local.json"),"utf8"));
const project=registry.projects?.staging,action=process.argv[2]||"";
...
const baselinePath=path.join(phaseDir,"staging-baseline.json");

if(!project?.ref||!project.databasePassword||project.name!=="HungrieApp Staging"||!project.url||!project.publishableKey||!managementToken||
  project.ref===registry.projects?.development?.ref||project.ref===registry.projects?.production?.ref)throw new Error("Safe isolated Staging configuration is required.");
...
if(!["backup","preflight","apply","types","probe","plans","prepare-device-fixtures","cleanup-device-fixtures","verify-cleanup"].includes(action))throw new Error("Use a reviewed Phase 8 action with both pinned migration checksums.");
...
const query=async sql=>{const response=await fetch(`https://api.supabase.com/v1/projects/${project.ref}/database/query`,{method:"POST",headers:{authorization:`Bearer ${managementToken}`,"content-type":"application/json"},body:JSON.stringify({query:sql})});if(!response.ok)throw new Error(`Staging SQL request failed (${response.status}); details withheld.`);const rows=await response.json();if(!Array.isArray(rows))throw new Error("Staging SQL response was malformed.");return rows};
const metadata=async()=>{const response=await fetch(`https://api.supabase.com/v1/projects/${project.ref}`,{headers:{authorization:`Bearer ${managementToken}`}});if(!response.ok)throw new Error(`Staging metadata request failed (${response.status}).`);const value=await response.json();if(value.name!=="HungrieApp Staging"||value.region!=="eu-central-1"||value.status!=="ACTIVE_HEALTHY"||!String(value.database?.version||"").startsWith("17"))throw new Error("Staging identity, region, health, or PostgreSQL version changed.");return value};
...
const snapshotSql=`select
  ...
  (select count(*)::integer from public.product_reviews where status='published') product_published,
  (select count(*)::integer from public.product_reviews where status='hidden') product_hidden,
...
const snapshot=async()=>{const [catalog]=await query("select to_regclass('private.order_review_reports') is not null v2");if(catalog.v2)return(await query(snapshotSql))[0];return(await query(`select
  (select count(*)::integer from public.product_reviews) product_reviews,
  (select count(*)::integer from public.product_reviews where status='published') product_published,
  (select count(*)::integer from public.product_reviews where status='hidden') product_hidden,
  (select count(*)::integer from public.order_reviews) order_reviews,0::integer reports,0::integer reactions,0::integer customer_operations,0::integer action_operations,
  (select count(*)::integer from (select review_key from public.product_reviews group by review_key having count(*)>1)x) product_duplicates,
  (select count(*)::integer from (select review_key from public.order_reviews group by review_key having count(*)>1)x) order_duplicates,
  (select encode(extensions.digest(coalesce(jsonb_agg(to_jsonb(r) order by id)::text,'[]'),'sha256'),'hex') from public.product_reviews r) product_digest,
  (select encode(extensions.digest(coalesce(jsonb_agg(to_jsonb(r) order by id)::text,'[]'),'sha256'),'hex') from public.order_reviews r) order_digest,
  encode(extensions.digest('[]','sha256'),'hex') report_digest,encode(extensions.digest('[]','sha256'),'hex') reaction_digest`))[0]};
const expectedBaseline=value=>Number(value.product_reviews)===6&&Number(value.product_published)===6&&Number(value.product_hidden)===0&&Number(value.order_reviews)===0&&Number(value.reports)===0&&Number(value.reactions)===0&&Number(value.product_duplicates)===0&&Number(value.order_duplicates)===0;
const dryRun=()=>{const result=run("supabase",["db","push","--project-ref",project.ref,"--password",project.databasePassword,"--skip-vault","--dry-run"],{env:cliEnv});const output=`${result.stdout||""}\n${result.stderr||""}`;const listed=[...new Set([...output.matchAll(/\d{14}_[A-Za-z0-9_]+\.sql/g)].map(match=>match[0]))];const expected=[baseMigration,migrationName];if(result.status!==0||listed.length!==2||expected.some(name=>!listed.includes(name))||listed.some(name=>!expected.includes(name)))throw new Error("Staging dry-run did not contain exactly the two reviewed migrations.");};
const preflight=async({persist=true}={})=>{await metadata();const versions=await history();if(versions.length!==45||String(versions.at(-1)?.version)!=="20260916130000")throw new Error("Staging migration history changed from the Phase 8 baseline.");const applied=new Set(versions.map(row=>String(row.version)));const pending=fs.readdirSync(path.join(root,"supabase/migrations")).filter(name=>/^\d{14}_.+\.sql$/.test(name)&&!applied.has(name.slice(0,14))).sort();if(pending.length!==2||pending[0]!==baseMigration||pending[1]!==migrationName)throw new Error("Pending migration set changed.");const before=await snapshot();if(!expectedBaseline(before))throw new Error("Staging review baseline changed; refusing to migrate.");dryRun();const [population]=await query("select (select count(*) from public.profiles)::integer profiles,(select count(*) from public.restaurants)::integer restaurants,(select count(*) from public.orders)::integer orders");if(Number(population.profiles)!==43||Number(population.restaurants)!==11||Number(population.orders)!==30)throw new Error("Staging unrelated-data population changed from the approved Phase 8 baseline.");const record={environment:"staging",projectRef:project.ref,capturedAt:new Date().toISOString(),migrations:[{name:baseMigration,sha256:baseSha},{name:migrationName,sha256:expectedSha}],migrationCount:45,latestMigration:"20260916130000",population,snapshot:before};if(persist)fs.writeFileSync(baselinePath,JSON.stringify(record,null,2),{mode:0o600});return record};
const assertApplied=async()=>{const versions=await history();if(versions.length!==47||String(versions.at(-1)?.version)!=="20260917110000")throw new Error("Staging Phase 8 migration is not applied.")};
...
if(action==="backup"){
  if(arg("--confirm")!=="staging:review-v2-phase8-backup")throw new Error("Backup confirmation mismatch.");await metadata();const directory=path.join(phaseDir,`backup-${new Date().toISOString().replaceAll(/[:.]/g,"-")}`);fs.mkdirSync(directory,{recursive:true,mode:0o700});const entries=[{name:"schema.sql",args:[]},{name:"data.sql",args:["--data-only","--use-copy"]}];for(const entry of entries){const output=path.join(directory,entry.name),result=run("supabase",["db","dump","--project-ref",project.ref,"--password",project.databasePassword,"--file",output,...entry.args],{env:cliEnv});if(result.status!==0||!fs.existsSync(output)||fs.statSync(output).size<1024)throw new Error("Restricted Staging backup failed.");fs.chmodSync(output,0o600)}const files=entries.map(({name})=>{const file=path.join(directory,name);return{name,bytes:fs.statSync(file).size,sha256:sha256(fs.readFileSync(file))}}),manifest={environment:"staging",projectRef:project.ref,recordedAt:new Date().toISOString(),migrations:[{name:baseMigration,sha256:baseSha},{name:migrationName,sha256:expectedSha}],files},manifestPath=path.join(directory,"manifest.json");fs.writeFileSync(manifestPath,JSON.stringify(manifest,null,2),{mode:0o600,flag:"wx"});console.log(JSON.stringify({action,environment:"staging",manifestPath,files}));
}else if(action==="preflight"){
  if(arg("--confirm")!=="staging:review-v2-phase8-preflight")throw new Error("Preflight confirmation mismatch.");const result=await preflight();console.log(JSON.stringify({action,environment:"staging",migrations:result.migrations,migrationCount:45,baseline:result.snapshot,dryRun:true}));
}else if(action==="apply"){
  if(arg("--confirm")!=="staging:review-v2-phase8-migration")throw new Error("Migration confirmation mismatch.");const before=await preflight(),manifestPath=path.resolve(arg("--backup-manifest")||"");if(!manifestPath.startsWith(phaseDir+path.sep)||!fs.existsSync(manifestPath))throw new Error("Fresh Phase 8 backup manifest is required.");const manifest=JSON.parse(fs.readFileSync(manifestPath,"utf8"));if(manifest.environment!=="staging"||manifest.projectRef!==project.ref||manifest.migrations?.[0]?.sha256!==baseSha||manifest.migrations?.[1]?.sha256!==expectedSha||Date.now()-Date.parse(manifest.recordedAt)>86_400_000)throw new Error("Backup manifest is stale or mismatched.");for(const entry of manifest.files||[]){const file=path.join(path.dirname(manifestPath),entry.name);if(!fs.existsSync(file)||sha256(fs.readFileSync(file))!==entry.sha256)throw new Error("Backup checksum verification failed.")}const pushed=run("supabase",["db","push","--project-ref",project.ref,"--password",project.databasePassword,"--skip-vault","--yes"],{env:cliEnv});if(pushed.status!==0)throw new Error("Staging migration application failed; operator output withheld.");await assertApplied();const after=await snapshot();for(const key of ["product_reviews","product_published","product_hidden","order_reviews","reports","reactions","customer_operations","action_operations","product_digest","order_digest","report_digest","reaction_digest"])if(before.snapshot[key]!==after[key])throw new Error(`Post-migration reconciliation failed for ${key}.`);const [catalog]=await query(`select
    ...
    from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='admin_list_order_review_reports_v2'`);if(!catalog?.audit_grant||!catalog?.report_grant||!catalog?.owner_ok||!catalog?.report_projection_ok)throw new Error("Post-migration catalog verification failed.");const lint=run("supabase",["db","lint","--linked","--project-ref",project.ref,"--schema","public,private,migration","--level","error","--fail-on","error"],{env:cliEnv});if(lint.status!==0)throw new Error("Hosted Staging database lint failed; output withheld.");fs.writeFileSync(path.join(phaseDir,"staging-applied.json"),JSON.stringify({appliedAt:new Date().toISOString(),migrations:[{name:baseMigration,sha256:baseSha},{name:migrationName,sha256:expectedSha}],before:before.snapshot,after,backupManifest:manifestPath,hostedLint:"clean"},null,2),{mode:0o600});console.log(JSON.stringify({action,environment:"staging",migrations:[baseMigration,migrationName],migrationCount:47,reconciled:true,authenticatedOnly:true,owner:"hungrie_api_owner",hostedLint:"clean"}));
}else if(action==="types"){
  if(arg("--confirm")!=="staging:review-v2-phase8-types")throw new Error("Type confirmation mismatch.");await assertApplied();const hosted=run("supabase",["gen","types","typescript","--project-id",project.ref,"--schema","public"],{env:cliEnv}),local=run("supabase",["gen","types","typescript","--local","--schema","public"]),tracked=fs.readFileSync(path.join(root,"packages/database-types/src/database.generated.ts"),"utf8");if(hosted.status!==0||local.status!==0||!publicSchema(hosted.stdout)||publicSchema(hosted.stdout)!==publicSchema(local.stdout)||publicSchema(hosted.stdout)!==publicSchema(tracked))throw new Error("Hosted, local, and checked-in public schema types differ.");const output=path.join(phaseDir,"staging-types.ts");fs.writeFileSync(output,hosted.stdout,{mode:0o600});console.log(JSON.stringify({action,environment:"staging",hostedSha256:sha256(hosted.stdout),localSha256:sha256(local.stdout),trackedSha256:sha256(tracked),publicSchemaSha256:sha256(publicSchema(hosted.stdout)),schemaParity:true,trackedChanged:false,evidencePath:output}));
}
...
if (action === "verify-cleanup") {
  if (arg("--confirm") !== "staging:review-v2-phase8-cleanup") throw new Error("Cleanup confirmation mismatch.");
  await assertApplied();
  if (!fs.existsSync(baselinePath)) throw new Error("Phase 8 preflight baseline is missing.");
  const baseline=JSON.parse(fs.readFileSync(baselinePath,"utf8")),current=await snapshot();
  const [probe]=await query(`select
    (select count(*) from public.profiles where id like 'crv2p8_%')::integer profiles,
    (select count(*) from public.orders where id like 'crv2p8_%')::integer orders,
    (select count(*) from public.order_reviews where id like 'crv2p8_%' or order_id like 'crv2p8_%')::integer reviews,
    (select count(*) from private.order_review_reports where review_id like 'crv2p8_%')::integer reports,
    (select count(*) from private.order_review_meal_reactions where review_id like 'crv2p8_%' or order_id like 'crv2p8_%')::integer reactions,
    (select count(*) from private.audit_log where target_id like 'crv2p8_%' or metadata::text like '%crv2p8_%')::integer audits`);
  ...
  for(const key of ["product_reviews","product_published","product_hidden","order_reviews","reports","reactions","customer_operations","action_operations","product_digest","order_digest","report_digest","reaction_digest"])if(baseline.snapshot[key]!==current[key])throw new Error(`Cleanup reconciliation failed for ${key}.`);
  if(Object.values(probe).some(Number)||!security?.audit_authenticated_only||!security?.audit_owner||!security?.audit_projection_safe)throw new Error("Phase 8 cleanup or security verification failed.");
  const result={action,environment:"staging",verifiedAt:new Date().toISOString(),migration:migrationName,migrationSha256:expectedSha,migrationCount:47,baseline:baseline.snapshot,current,probeRows:probe,baselineReconciled:true,security};
  fs.writeFileSync(path.join(phaseDir,"staging-qualified.json"),JSON.stringify(result,null,2),{mode:0o600});
  console.log(JSON.stringify({action,environment:"staging",migrationCount:47,probeRows:probe,baselineReconciled:true,security}));
}

if (action === "probe") {
  if (arg("--confirm") !== "staging:review-v2-phase8-probe") throw new Error("Probe confirmation mismatch.");
  ...
  if(versions.length!==47||String(versions.at(-1)?.version)!=="20260917110000")throw new Error("Staging v2 migration is unavailable.");
  ...
  const app=admin.initializeApp({credential:admin.credential.cert(credential),projectId:credential.project_id},`crv2p8-${Date.now()}`),auth=app.auth();
  const suffix=crypto.randomUUID().replaceAll("-","").slice(0,12),prefix=`crv2p8_${suffix}`;
  ...
  try{
    ...
    if(restaurants.length<2)throw new Error("Two active Staging Restaurants are required.");[restaurantA,restaurantB]=restaurants.map(x=>x.id);
...
if(action==="prepare-device-fixtures"||action==="cleanup-device-fixtures"){
  const preparing=action==="prepare-device-fixtures";
  const confirmation=preparing?"staging:review-v2-phase8-device-prepare":"staging:review-v2-phase8-device-cleanup";
  if(arg("--confirm")!==confirmation)throw new Error("Device-fixture confirmation mismatch.");
  await assertApplied();
  const manifestPath=path.join(phaseDir,"device-fixtures.json");
  const credentialPath=path.resolve(arg("--credential")||"");
  if(!credentialPath||!fs.existsSync(credentialPath)||!path.relative(root,credentialPath).startsWith(".."))throw new Error("An external Firebase Admin credential is required.");
  const credential=JSON.parse(fs.readFileSync(credentialPath,"utf8"));
  if(credential.project_id!=="hungrieapp-a2288")throw new Error("Firebase credential project mismatch.");
  const require=createRequire(import.meta.url),admin=require(path.join(root,"functions/node_modules/firebase-admin"));
  const app=admin.initializeApp({credential:admin.credential.cert(credential),projectId:credential.project_id},`crv2p8-device-${Date.now()}`),auth=app.auth();
  try{
    if(preparing){
      if(fs.existsSync(manifestPath))throw new Error("Device fixtures already exist; clean them before preparing another set.");
      const iosBuildId=arg("--ios-build-id"),androidBuildId=arg("--android-build-id");
      if(!iosBuildId||!androidBuildId)throw new Error("Successful iOS and Android EAS build IDs are required before device fixtures are created.");
      const inspectBuild=(id,platform)=>{const result=run("npx",["eas-cli@16.32.0","build:view",id,"--json","--non-interactive"],{env:process.env});if(result.status!==0)throw new Error(`${platform} build inspection failed.`);const start=result.stdout.indexOf("{");const value=JSON.parse(result.stdout.slice(start));const versionOk=platform==="ios"?String(value.appBuildVersion||value.buildNumber)==="39":String(value.appBuildVersion||value.versionCode)==="38";if(value.status!=="FINISHED"||value.platform?.toLowerCase()!==platform||value.distribution!=="INTERNAL"||value.buildProfile!=="preview"||!versionOk)throw new Error(`${platform} build does not match the reviewed Phase 8 preview contract.`);return{id:value.id,platform,status:value.status,profile:value.buildProfile,distribution:value.distribution,appVersion:value.appVersion,appBuildVersion:value.appBuildVersion||value.buildNumber||value.versionCode,projectId:value.project?.id||value.projectId}};
      const builds={ios:inspectBuild(iosBuildId,"ios"),android:inspectBuild(androidBuildId,"android")};
      if(builds.ios.appVersion!=="1.0.2"||builds.android.appVersion!=="1.0.2"||builds.ios.projectId!=="be624db8-7de1-4b8b-bd86-605431679967"||builds.android.projectId!=="be624db8-7de1-4b8b-bd86-605431679967")throw new Error("Preview project or marketing version mismatch.");
      const web=JSON.parse(fs.readFileSync(path.join(secure,"admin-firebase-web-development.local.json"),"utf8"));
      const firebaseRequest=async(version,method,body)=>{const response=await fetch(`https://identitytoolkit.googleapis.com/${version}/${method}?key=${encodeURIComponent(web.apiKey)}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});const payload=await response.json().catch(()=>null);if(!response.ok)throw new Error(`Firebase ${method} failed (${response.status}).`);return payload};
      const alphabet="ABCDEFGHIJKLMNOPQRSTUVWXYZ234567",base32=value=>{let bits="";for(const c of value.replaceAll("=","").toUpperCase()){const i=alphabet.indexOf(c);if(i<0)throw new Error("Invalid TOTP secret.");bits+=i.toString(2).padStart(5,"0")}return Buffer.from(bits.match(/.{8}/g)?.map(x=>Number.parseInt(x,2))??[])};
      const totp=(secret,period=30,digits=6,algorithm="SHA1")=>{const input=Buffer.alloc(8);input.writeBigUInt64BE(BigInt(Math.floor(Date.now()/1000/period)));const digest=crypto.createHmac(algorithm.toLowerCase().replace("hmac",""),base32(secret)).update(input).digest(),offset=digest[digest.length-1]&15;return((digest.readUInt32BE(offset)&0x7fffffff)%(10**digits)).toString().padStart(digits,"0")};
      const suffix=crypto.randomUUID().replaceAll("-","").slice(0,10),prefix=`crv2p8_device_${suffix}`,password=`P8!${crypto.randomBytes(20).toString("base64url")}`;
      const ids={customerA:`${prefix}_ca`,customerB:`${prefix}_cb`,suspended:`${prefix}_sus`,revoked:`${prefix}_rev`,restaurantA:`${prefix}_ra`,restaurantB:`${prefix}_rb`,admin:`${prefix}_admin`,category:`${prefix}_cat`,itemA:`${prefix}_item_a`,itemB:`${prefix}_item_b`,itemC:`${prefix}_item_c`,main:`${prefix}_main`,repeat:`${prefix}_repeat`,older:`${prefix}_older`,cross:`${prefix}_cross`,expired:`${prefix}_expired`};
      const [restaurantA,restaurantB]=await query("select id from public.restaurants where lifecycle_status='active' order by id limit 2");if(!restaurantA||!restaurantB)throw new Error("Two active Staging Restaurants are required.");
      const users={};
      try{
        for(const [role,id]of Object.entries({customerA:ids.customerA,customerB:ids.customerB,suspended:ids.suspended,revoked:ids.revoked,restaurantA:ids.restaurantA,restaurantB:ids.restaurantB,unmapped:`${prefix}_unmapped`,admin:ids.admin})){users[role]=await auth.createUser({email:`${id}@example.invalid`,emailVerified:true,password});await auth.setCustomUserClaims(users[role].uid,{role:"authenticated"})}
        const profileRows=Object.entries({customerA:ids.customerA,customerB:ids.customerB,suspended:ids.suspended,revoked:ids.revoked,restaurantA:ids.restaurantA,restaurantB:ids.restaurantB,admin:ids.admin}).map(([role,id])=>`(${quote(id)},${quote(users[role].uid)},'Phase 8 device',${quote(`${id}@example.invalid`)})`).join(",");
        await query(`begin;
          insert into public.profiles(id,firebase_uid,name,email) values ${profileRows};
          insert into private.account_access(profile_id,account_type,status,activated_at)values(${quote(ids.customerA)},'customer','active',transaction_timestamp()),(${quote(ids.customerB)},'customer','active',transaction_timestamp());
          insert into private.account_access(profile_id,account_type,status,suspended_at)values(${quote(ids.suspended)},'customer','suspended',transaction_timestamp());
          insert into private.account_access(profile_id,account_type,status,revoked_at)values(${quote(ids.revoked)},'customer','revoked',transaction_timestamp());
          insert into private.account_access(profile_id,account_type,status,activated_at,restaurant_id,restaurant_role)values(${quote(ids.restaurantA)},'restaurant','active',transaction_timestamp(),${quote(restaurantA.id)},'manager'),(${quote(ids.restaurantB)},'restaurant','active',transaction_timestamp(),${quote(restaurantB.id)},'manager');
          insert into private.account_access(profile_id,account_type,status,activated_at,admin_role,admin_mfa_enrolled_at)values(${quote(ids.admin)},'admin','active',transaction_timestamp(),'admin',transaction_timestamp());
          insert into public.categories(id,restaurant_id,name,is_active,sort_order)values(${quote(ids.category)},${quote(restaurantA.id)},'Phase 8 device',true,99999);
          insert into public.menu_items(id,restaurant_id,category_id,name,price_kurus,is_active,sort_order)values(${quote(ids.itemA)},${quote(restaurantA.id)},${quote(ids.category)},'Device meal A',100,true,0),(${quote(ids.itemB)},${quote(restaurantA.id)},${quote(ids.category)},'Device meal B',100,true,1),(${quote(ids.itemC)},${quote(restaurantA.id)},${quote(ids.category)},'Device meal C',100,true,2);
          insert into public.orders(id,profile_id,restaurant_id,status,payment_method,subtotal_kurus,total_kurus,delivered_at,created_at)values
            (${quote(ids.main)},${quote(ids.customerA)},${quote(restaurantA.id)},'delivered','cash',400,400,transaction_timestamp()-interval '1 hour',transaction_timestamp()-interval '2 hours'),
            (${quote(ids.repeat)},${quote(ids.customerA)},${quote(restaurantA.id)},'delivered','cash',100,100,transaction_timestamp()-interval '2 hours',transaction_timestamp()-interval '3 hours'),
            (${quote(ids.older)},${quote(ids.customerA)},${quote(restaurantA.id)},'delivered','cash',100,100,transaction_timestamp()-interval '3 hours',transaction_timestamp()-interval '4 hours'),
            (${quote(ids.cross)},${quote(ids.customerB)},${quote(restaurantA.id)},'delivered','cash',100,100,transaction_timestamp()-interval '90 minutes',transaction_timestamp()-interval '2 hours'),
            (${quote(ids.expired)},${quote(ids.customerA)},${quote(restaurantA.id)},'delivered','cash',100,100,transaction_timestamp()-interval '30 days 1 second',transaction_timestamp()-interval '31 days');
          insert into public.order_items(id,order_id,menu_item_id,source_menu_item_id,name_snapshot,unit_price_kurus,quantity,customizations_snapshot)values
            (${quote(`${ids.main}_1`)},${quote(ids.main)},${quote(ids.itemA)},${quote(ids.itemA)},'Device meal A',100,1,'[]'),(${quote(`${ids.main}_2`)},${quote(ids.main)},${quote(ids.itemA)},${quote(ids.itemA)},'Device meal A',100,2,'[{"size":"large"}]'),(${quote(`${ids.main}_3`)},${quote(ids.main)},${quote(ids.itemB)},${quote(ids.itemB)},'Device meal B',100,1,'[]'),(${quote(`${ids.main}_4`)},${quote(ids.main)},${quote(ids.itemC)},${quote(ids.itemC)},'Device meal C',100,1,'[]'),
            (${quote(`${ids.repeat}_1`)},${quote(ids.repeat)},${quote(ids.itemA)},${quote(ids.itemA)},'Device meal A',100,1,'[]'),(${quote(`${ids.older}_1`)},${quote(ids.older)},${quote(ids.itemB)},${quote(ids.itemB)},'Device meal B',100,1,'[]'),(${quote(`${ids.cross}_1`)},${quote(ids.cross)},${quote(ids.itemA)},${quote(ids.itemA)},'Device meal A',100,1,'[]'),(${quote(`${ids.expired}_1`)},${quote(ids.expired)},${quote(ids.itemA)},${quote(ids.itemA)},'Device meal A',100,1,'[]');commit;`);
        const first=await firebaseRequest("v1","accounts:signInWithPassword",{email:`${ids.admin}@example.invalid`,password,returnSecureToken:true}),enrollment=await firebaseRequest("v2","accounts/mfaEnrollment:start",{idToken:first.idToken,totpEnrollmentInfo:{}}),session=enrollment.totpSessionInfo;
        await firebaseRequest("v2","accounts/mfaEnrollment:finalize",{idToken:first.idToken,displayName:"Phase 8 device",totpVerificationInfo:{sessionInfo:session.sessionInfo,verificationCode:totp(session.sharedSecretKey,session.periodSec,session.verificationCodeLength,session.hashingAlgorithm)}});
        const accounts=Object.fromEntries(Object.entries(users).map(([role,user])=>[role,{email:user.email,password}]));
        const manifest={environment:"staging",createdAt:new Date().toISOString(),prefix,builds,accounts,totp:{secret:session.sharedSecretKey,period:session.periodSec,digits:session.verificationCodeLength,algorithm:session.hashingAlgorithm,uri:`otpauth://totp/Hungrie:${encodeURIComponent(users.admin.email)}?secret=${session.sharedSecretKey}&issuer=Hungrie&period=${session.periodSec}&digits=${session.verificationCodeLength}`},ids,restaurants:{restaurantA:restaurantA.id,restaurantB:restaurantB.id}};
        fs.writeFileSync(manifestPath,JSON.stringify(manifest,null,2),{mode:0o600,flag:"wx"});
        console.log(JSON.stringify({action,environment:"staging",prefix,builds,accounts:Object.fromEntries(Object.entries(accounts).map(([role,value])=>[role,{email:value.email}])) ,manifestPath,secretsLogged:false}));
      }catch(error){for(const user of Object.values(users))if(user?.uid)await auth.deleteUser(user.uid).catch(()=>{});await query(`begin;delete from private.account_access where profile_id like ${quote(`${prefix}%`)};delete from public.order_items where order_id like ${quote(`${prefix}%`)};delete from public.orders where id like ${quote(`${prefix}%`)};delete from public.menu_items where id like ${quote(`${prefix}%`)};delete from public.categories where id like ${quote(`${prefix}%`)};delete from public.profiles where id like ${quote(`${prefix}%`)};commit;`).catch(()=>{});throw error}
    }else{
      if(!fs.existsSync(manifestPath))throw new Error("Device fixture manifest is missing.");const manifest=JSON.parse(fs.readFileSync(manifestPath,"utf8")),prefix=manifest.prefix;if(!String(prefix).startsWith("crv2p8_device_"))throw new Error("Device fixture prefix is invalid.");
      await query(`begin;delete from private.review_action_operations where actor_profile_id like ${quote(`${prefix}%`)};delete from private.order_review_reports where review_id in(select id from public.order_reviews where order_id like ${quote(`${prefix}%`)});delete from private.order_review_meal_reactions where order_id like ${quote(`${prefix}%`)};delete from private.customer_review_operations where profile_id like ${quote(`${prefix}%`)};delete from private.audit_log where actor_profile_id like ${quote(`${prefix}%`)} or target_id like ${quote(`${prefix}%`)} or metadata::text like ${quote(`%${prefix}%`)};delete from public.order_reviews where order_id like ${quote(`${prefix}%`)};delete from private.notification_deliveries where event_id in(select id from private.notification_events where order_id like ${quote(`${prefix}%`)});delete from private.notification_events where order_id like ${quote(`${prefix}%`)};delete from private.restaurant_order_visibility where order_id like ${quote(`${prefix}%`)};delete from private.customer_order_operations where order_id like ${quote(`${prefix}%`)};delete from private.order_status_history where order_id like ${quote(`${prefix}%`)};delete from private.order_contacts where order_id like ${quote(`${prefix}%`)};delete from public.order_items where order_id like ${quote(`${prefix}%`)};delete from public.orders where id like ${quote(`${prefix}%`)};delete from public.menu_items where id like ${quote(`${prefix}%`)};delete from public.categories where id like ${quote(`${prefix}%`)};delete from private.account_access where profile_id like ${quote(`${prefix}%`)};delete from public.profiles where id like ${quote(`${prefix}%`)};commit;`);
      for(const account of Object.values(manifest.accounts||{})){const user=await auth.getUserByEmail(account.email).catch(()=>null);if(user)await auth.deleteUser(user.uid)}
      const remaining=[];let pageToken;do{const page=await auth.listUsers(1000,pageToken);remaining.push(...page.users.filter(user=>user.email?.startsWith(prefix)));pageToken=page.pageToken}while(pageToken);if(remaining.length)throw new Error("Firebase device-fixture cleanup was incomplete.");
      fs.writeFileSync(manifestPath,JSON.stringify({environment:"staging",cleanedAt:new Date().toISOString(),prefix,builds:manifest.builds,credentialsRemoved:true},null,2),{mode:0o600});console.log(JSON.stringify({action,environment:"staging",prefix,databaseCleaned:true,firebaseCleaned:true,credentialsRemoved:true}));
    }
  }finally{await app.delete()}
}

if(action==="plans"){
  if(arg("--confirm")!=="staging:review-v2-phase8-plans")throw new Error("Query-plan confirmation mismatch.");
  const versions=await history();if(versions.length!==47||String(versions.at(-1)?.version)!=="20260917110000")throw new Error("Staging v2 migration is unavailable.");
  const planSql=`begin;
    create temporary table crv2p8_restaurants as select row_number()over(order by id)::integer n,id from public.restaurants where lifecycle_status='active' order by id limit 9;
    do $$begin if (select count(*) from crv2p8_restaurants)<2 then raise exception 'representative plans require two Restaurants';end if;end$$;
    create temporary table crv2p8_settings as select count(*)::integer restaurant_count,(select id from crv2p8_restaurants where n=1) target_restaurant from crv2p8_restaurants;
    insert into public.profiles(id,name,email,preferred_language)
      select 'crv2p8_plan_profile_'||g,'Plan fixture','crv2p8-plan-'||g||'@example.invalid','en' from generate_series(1,5000)g;
    insert into public.orders(id,profile_id,restaurant_id,status,payment_method,subtotal_kurus,total_kurus,delivered_at,created_at)
      select 'crv2p8_plan_order_'||g,'crv2p8_plan_profile_'||g,r.id,'delivered','cash',100,100,
        transaction_timestamp()-interval '1 hour',transaction_timestamp()-(g||' seconds')::interval
      from generate_series(1,5000)g join crv2p8_settings s on true join crv2p8_restaurants r on r.n=1+((g-1)%s.restaurant_count);
    insert into public.order_reviews(id,review_key,order_id,restaurant_id,profile_id,user_name_snapshot,restaurant_name_snapshot,
      ...
      select 'crv2p8_plan_review_'||g,'crv2p8_plan_key_'||g,'crv2p8_plan_order_'||g,r.id,'crv2p8_plan_profile_'||g,
    ...
    insert into private.order_review_meal_reactions(review_id,order_id,restaurant_id,profile_id,menu_item_id,reaction,menu_item_name_snapshot)
      select 'crv2p8_plan_review_'||g,'crv2p8_plan_order_'||g,r.id,'crv2p8_plan_profile_'||g,
        'crv2p8_plan_item_'||(g%100),case when g%2=0 then 'liked'::private.meal_reaction else 'disliked'::private.meal_reaction end,'Plan Item'
      from generate_series(1,5000)g join crv2p8_settings s on true join crv2p8_restaurants r on r.n=1+((g-1)%s.restaurant_count);
    insert into private.customer_review_operations(profile_id,operation_id,request_sha256,review_id)
      select 'crv2p8_plan_profile_'||g,md5('crv2p8-operation-'||g)::uuid,repeat('a',64),'crv2p8_plan_review_'||g from generate_series(1,5000)g;
    insert into private.order_review_reports(review_id,restaurant_id,created_by_profile_id,reason,status,created_at)
      select 'crv2p8_plan_review_'||g,r.id,'crv2p8_plan_profile_'||g,'spam',
      ...
      from generate_series(1,1500)g join crv2p8_settings s on true join crv2p8_restaurants r on r.n=1+((g-1)%s.restaurant_count);
    insert into private.audit_log(actor_profile_id,action,target_type,target_id,metadata,created_at)
      select 'crv2p8_plan_profile_1','order_review.report_status_changed_v2','order_review_report',
        (select id::text from private.order_review_reports where review_id='crv2p8_plan_review_1'),
        jsonb_build_object('contract_version',2,'restaurant_id',(select target_restaurant from crv2p8_settings),'operation_id',md5('crv2p8-audit-'||g)::uuid),
    ...
    create temporary table crv2p8_plans(name text primary key,expected_index text not null,plan jsonb not null);
    do $$declare p jsonb;target text:=(select target_restaurant from crv2p8_settings);begin
      ...
      insert into crv2p8_plans values('public_feed','order_reviews_published_feed_idx',p);
      execute 'explain(analyze,buffers,format json)select id from public.order_reviews where order_id=''crv2p8_plan_order_4999'' and profile_id=''crv2p8_plan_profile_4999'''into p;
      insert into crv2p8_plans values('customer_lookup','order_reviews_profile_created_idx',p);
      ...
      insert into crv2p8_plans values('restaurant_queue','order_reviews_restaurant_status_created_id_idx',p);
      ...
      insert into crv2p8_plans values('report_queue','order_review_reports_restaurant_status_created_id_idx',p);
      execute 'explain(analyze,buffers,format json)select id from private.audit_log where target_type=''order_review_report'' and target_id=(select id::text from private.order_review_reports where review_id=''crv2p8_plan_review_1'') order by created_at desc,id desc limit 20' into p;
      insert into crv2p8_plans values('report_audit','audit_log_target_created_idx',p);
      execute 'explain(analyze,buffers,format json)select review_id from private.customer_review_operations where profile_id=''crv2p8_plan_profile_4999'' and operation_id=md5(''crv2p8-operation-4999'')::uuid'into p;
      insert into crv2p8_plans values('operation_replay','customer_review_operations_pkey',p);
      ...
      insert into crv2p8_plans values('reaction_aggregate','order_review_reactions_restaurant_item_reaction_idx',p);
    end$$;
    select name,expected_index,plan::text like '%'||expected_index||'%' uses_expected_index,
      ...
      plan from crv2p8_plans order by name;
    rollback;`;
  const plans=await query(planSql);
  const output=path.join(phaseDir,"staging-query-plans.json");fs.writeFileSync(output,JSON.stringify({environment:"staging",capturedAt:new Date().toISOString(),volume:{reviews:5000,reactions:5000,reports:1500,operations:5000,audits:5000},plans},null,2),{mode:0o600});
  ...
  console.log(JSON.stringify({action,environment:"staging",volume:{reviews:5000,reactions:5000,reports:1500,operations:5000,audits:5000},plans:plans.map(({name,expected_index,uses_expected_index,large_seq_scan})=>({name,expectedIndex:expected_index,usesExpectedIndex:uses_expected_index,largeSeqScan:large_seq_scan})),rolledBack:true,evidencePath:output}));
}
