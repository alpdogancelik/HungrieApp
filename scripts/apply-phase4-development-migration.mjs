...
const migrations = ['20260913100000_phase4_admin_minimum.sql'];
const arg = (name) => process.argv.find((value) => value.startsWith(`${name}=`))?.slice(name.length + 1);
const apply = process.argv.includes('--apply');
if (arg('--target') !== 'development' || (apply && arg('--confirm') !== 'development:phase4-migration')) {
  throw new Error('Target development explicitly; apply requires --confirm=development:phase4-migration.');
}
...
if (history.length !== 30 || pending.length !== 1 ||
    migrations.some((name,index) => pending[index] !== name)) {
  throw new Error('Expected 30 migrations applied and only one Phase 4 migration pending in development.');
}
if (apply) {
...
if (after?.migrations !== 31 || after.account_rows !== 60) {
