...
if (arg('--target') !== 'staging' || (apply && arg('--confirm') !== 'staging:phase3-migration')) {
  throw new Error('Target staging explicitly; apply requires --confirm=staging:phase3-migration.');
}
...
if (!project?.ref || !project.databasePassword || project.name !== 'HungrieApp Staging' ||
    project.ref === state.projects?.development?.ref || project.ref === state.projects?.production?.ref) {
  throw new Error('Staging identity is incomplete or overlaps another environment.');
}
...
const query = async (sql) => {
  ...
  if (!response.ok) throw new Error(`Staging SQL preflight failed (${response.status}).`);
  return response.json();
};
const metadataResponse = await fetch(`https://api.supabase.com/v1/projects/${project.ref}`, {
  headers: { authorization: `Bearer ${token}` }
});
if (!metadataResponse.ok) throw new Error(`Staging metadata check failed (${metadataResponse.status}).`);
...
if (history.length !== 28 || pending.length !== 2 ||
  ...
  throw new Error('Expected 28 migrations applied and only two Phase 3 migrations pending in staging.');
}
if (apply) {
...
if (dry.status !== 0 || migrations.some((name) => !`${dry.stdout}\n${dry.stderr}`.includes(name))) {
  throw new Error('Staging migration dry-run failed.');
}
if (!apply) {
  console.log(JSON.stringify({ target: 'staging', mode: 'dry-run', migrations, sha256: sha }));
  process.exit(0);
}
...
if (after?.migrations !== 30 || after.account_rows !== 0 || after.runs !== 0) {
  throw new Error('Staging post-migration invariant failed.');
}
console.log(JSON.stringify({ target: 'staging', mode: 'applied', migrations, sha256: sha,
