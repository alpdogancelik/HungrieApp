...
| CUST-14 | Orders list uses distinct rounded white cards with a subtle outer border/shadow like the old web version; the full-width upper/header area stays white and opaque while cards scroll beneath it, with no card-border bleed-through, unwanted gray internal lines, or visible sticky-header shadow; content and actions remain readable |  |  |
...
|---|---|---|
| CWEB-01 | Orders retain the original white rounded cards, subtle outer borders/shadows, status, cancellation reason, and actions | Pending browser observation |
