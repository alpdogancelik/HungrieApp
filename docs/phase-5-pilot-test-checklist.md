...
| ORD-04 | Open the same pending order in two tabs and submit the same transition at nearly the same time. | One tab succeeds. The other reports that the order changed and cannot create a duplicate transition. |  |
